...
def compute_loss_on_test(encoder, ode_func, classifier, args, dataloader, n_batches, device, phase, latent_dim=6, AMT_index=-2, labels_n=1):
    ptnms = []
    ...
    for itr in range(n_batches):
        ...
        if phase == "test":
            # Create the mask across the third dimension (for multi-label)
            idx_not_nan = ~(torch.isnan(labels) | (labels == -1))
            
            # Apply the mask to filter out invalid labels and predictions
            preds = preds[idx_not_nan].view(-1, labels.shape[-1])
            labels = labels[idx_not_nan].view(-1, labels.shape[-1])

            # Use the mask on the 'times' tensor, keeping only valid entries
            times_mask = idx_not_nan.any(dim=2).flatten()  # 1D mask aligned with `times`
            times = times[times_mask]

            # Extend `ptnms`, `Times`, `predictions`, and `ground_truth` with valid filtered values
    ...
    if phase == "test":
        #ground_truth shape [n_samples, n_labels]
        result_dict = {
            "PTNM": ptnms,
            "TIME": Times,
            "loss": rmse_loss
        ...
        # Add each label and prediction as separate keys in the dictionary
        for i in range(ground_truth.shape[1]):
            result_dict[f"label_{i+1}"] = ground_truth[:, i].cpu().tolist()
            result_dict[f"pred_{i+1}"] = predictions[:, i].cpu().tolist()
...
def compute_loss_on_interp(encoder, ode_func, classifier, args, dataloader, dataloader_o, n_batches, device, phase, latent_dim = 6, AMT_index=-2):
    ptnms = []
    Times = torch.Tensor([]).to(device=device)
    predictions = torch.Tensor([]).to(device=device)
    ground_truth = torch.Tensor([]).to(device=device)
